...
export const ListboxComponent: React.FC<ListboxComponentProps> = (props) => (
    <Listbox value={props.value} onChange={props.onChange}>
      <div className="relative w-full">
        <Listbox.Button className={`relative w-full cursor-default bg-zinc-950 py-2 pl-3 pr-10 text-left ${props.classname}`}>
                  ...
                  <>
                    ...
                    {selected && (
                      <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-amber-600">
                        <Check className="h-5 w-5" aria-hidden="true" weight='regular' />
                      </span>
                    )}
                  </>
                )}
